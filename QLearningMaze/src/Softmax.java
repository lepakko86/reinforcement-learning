		...
		for(int i =0 ; i<NUMBER_OF_ROW_COLUMN;i++){
			for(int j =0 ; j<NUMBER_OF_ROW_COLUMN ;j++) {
				...
				if(board[i][j]==0) {
					...
					if(j>0 && i<NUMBER_OF_ROW_COLUMN-1) {//sol asagý
						if(board[i][j-1]== 0 && board[i+1][j]== 0)
							R[NUMBER_OF_ROW_COLUMN*i+j][NUMBER_OF_ROW_COLUMN*(i+1)+j-1]=0;
						else
							R[NUMBER_OF_ROW_COLUMN*i+j][NUMBER_OF_ROW_COLUMN*(i+1)+j-1]=-1;
					...
					if(j<NUMBER_OF_ROW_COLUMN-1 && i>0) { //sag yukarý
						if(board[i][j+1]== 0 && board[i-1][j]== 0)
							R[NUMBER_OF_ROW_COLUMN*i+j][NUMBER_OF_ROW_COLUMN*(i-1)+j+1]=0;
						else
							R[NUMBER_OF_ROW_COLUMN*i+j][NUMBER_OF_ROW_COLUMN*(i-1)+j+1]=-1;
					...
					if(j<NUMBER_OF_ROW_COLUMN-1 && i<NUMBER_OF_ROW_COLUMN-1 ) { //sag asagý
						if(board[i][j+1]== 0 && board[i+1][j]== 0)
							R[NUMBER_OF_ROW_COLUMN*i+j][NUMBER_OF_ROW_COLUMN*(i+1)+j+1]=0;
						else
							R[NUMBER_OF_ROW_COLUMN*i+j][NUMBER_OF_ROW_COLUMN*(i+1)+j+1]=-1;
	...
	public static int Q(int state , int action){
		...
		if(action==LEFT){
			...
			do{
				...
				if(random<0.8)//right
					newstate= state+1;
				else if(random<0.9   )//yukarý sað
					newstate=state-NUMBER_OF_ROW_COLUMN+1;
				else 
					newstate= state+NUMBER_OF_ROW_COLUMN+1;//asaðý sað
